            ...
            fs.readFile(oldPath, function (err, data) {
                ...
                new sql.ConnectionPool(database.config).connect().then(pool => {
                    ...
                    if (result.rowsAffected[2]) {
                        rowsAffected = result.rowsAffected[2]
                    } else {
                        rowsAffected = result.rowsAffected
                    }

                    if(result.rowsAffected[0] > 0) {
                        res.render('success', {title: "Success", type: 'created', rowsAffected: rowsAffected})
                    } else {
                        res.render('success', {title: "Success", type: 'created', rowsAffected: rowsAffected, isZero: true})
                    }
                }).catch(err => {
